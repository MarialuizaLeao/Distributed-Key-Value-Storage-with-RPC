...
def run():
    ...
    while True:
        ...
        if command == "I":
            ...
            response = stub.insert(messages.insertRequest(key = int(key), value = str(value))) # request to the server
            ...
            """
            key = clientInput[1] # ch
            response = stub.consult(messages.consultRequest(key = int(key))) # request to the server
            ...
            response = stub.activate(messages.activateRequest(centralServerID = str(service))) # request to the server
            ...
            response = stub.terminate(messages.Empty()) # request to the server

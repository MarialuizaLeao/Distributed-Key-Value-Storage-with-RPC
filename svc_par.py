...
class Server(services.serverServicer):
    ...
    def activate(self, request, context) -> int:
        ...
        if not self.flag:
            ...
            stub = centralServices.centralServerStub(channel) # stub is the object with references to the remote procedures
            response = stub.register(centralMessages.registerRequest(serverID = str(self.ID), keyList = list(self.pairs.keys()))) # request to the central server
            return messages.activateResponse(amountOfActivatedKeys = int(response.amountOfRegisteredKeys)) # response from the central server
    ...
    def terminate(self, request, context):
        ...
        return messages.terminateResponse(key = 0) # return the number of keys that were active at the time of termination

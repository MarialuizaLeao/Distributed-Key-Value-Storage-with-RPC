...
class centralServer(centralServer_pb2_grpc.centralServerServicer):
    ...
    def map(self, request, context) -> int:
        ...
        if key in self.serversDict:
            return centralServer_pb2.mapResponse(serverID = self.serversDict[key])
        else:
            return centralServer_pb2.mapResponse(serverID = "")
    ...
    def terminate(self, request, context):
        ...
        return centralServer_pb2.terminateResponse(amountOfRegisteredKeys = len(self.serversDict))
...
def run():
    ...
    centralServer_pb2_grpc.add_centralServerServicer_to_server(centralServer(stop_event), server)
